...
app.post('/mybills/datapost', (req, res) => {
  ...
  connection.query('INSERT INTO mybills ( BillerId, BillerName, Location, Power_Kwh, SanctionedLoad, DueDate, Amount, BillStatus, CL_ID) VALUES (?, ?,?, ?,?, ?,?, ?, ?)', [BillerId, BillerName, Location, Power_Kwh, SanctionedLoad, DueDate, Amount, BillStatus, CL_ID], (err, result) => {
    if (err) {
      console.error('Error executing query:', err);
      res.status(500).json({ error: 'Internal Server Error' });
      return;
...
app.post('/location/datas', verifyToken, (req, res) => {
  ...
  connection.query(`SELECT * FROM mybills where CL_ID=${CL_ID} ORDER BY my_b ASC`, (err, rows) => {
    ...
    connection.query(`SELECT * FROM client_list where CL_ID=${CL_ID}`, (err, rows1) => {
      if(err) {
        res.status(500).json({ error: 'Internal Server Error' });
      }
      res.json({clientdata: rows1, data: rows, user: req.user});
    });
...
app.post('/per/location/datas', verifyToken, (req, res) => {
  ...
  connection.query(`SELECT * FROM bill_data where unique_identifier= "${unique_identifier}" ORDER BY id ASC`, (err, rows) => {
    if (err) {
      console.error('Error executing query:', err);
      res.status(500).json({ error: 'Internal Server Error' });
      return;
